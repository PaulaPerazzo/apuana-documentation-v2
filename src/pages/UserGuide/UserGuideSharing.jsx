...
const UserGuideSharing = () => {
  return (
    <div className="space-y-8">
      <h1 className="text-3xl font-bold text-gray-900 border-b pb-4">Compartilhando Dados com ACLs</h1>
      ...
      <section className="space-y-4">
        <h2 className="text-2xl font-semibold text-gray-800">Passo a Passo</h2>
        
        <h3 className="text-xl font-medium text-gray-800">1. Garantir acesso a si mesmo (arquivos futuros)</h3>
        <CodeBlock language="bash">
          {`setfacl -Rdm user:\${USER}:rwx /network/scratch/\${USER:0:1}/\${USER}/X/Y/Z/`}
        </CodeBlock>

        <h3 className="text-xl font-medium text-gray-800">2. Garantir acesso ao outro usuário (arquivos futuros)</h3>
        <CodeBlock language="bash">
          {`setfacl -Rdm user:\${USER2}:rwx /network/scratch/\${USER:0:1}/\${USER}/X/Y/Z/`}
        </CodeBlock>

        <h3 className="text-xl font-medium text-gray-800">3. Garantir acesso ao outro usuário (arquivos existentes)</h3>
        <CodeBlock language="bash">
          {`setfacl -Rm user:\${USER2}:rwx /network/scratch/\${USER:0:1}/\${USER}/X/Y/Z/`}
        </CodeBlock>

        <h3 className="text-xl font-medium text-gray-800">4. Permissão de pesquisa (execução) nas pastas pai</h3>
        <CodeBlock language="bash">
...
setfacl -m user:\${USER2}:x /network/scratch/\${USER:0:1}/\${USER}/`}
      ...
      <section className="space-y-4">
        <h2 className="text-2xl font-semibold text-gray-800">Visualizando ACLs</h2>
